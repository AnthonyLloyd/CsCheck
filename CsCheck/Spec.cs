...
public static class Spec
{
    ...
    /// <remarks>BFS deduplication uses the state's <see cref="object.Equals(object?)"/> and <see cref="object.GetHashCode"/>. If <typeparamref name="S"/> contains a field whose equality is not structural — for example a reference type, a collection, or a value type where all fields can be zero so distinct instances appear equal — Exhaustive will visit the same logical state multiple times, producing an inflated state count and potentially running indefinitely without closing. Use primitive fields, bitmasks, or <see cref="System.Collections.Immutable.ImmutableHashSet{T}"/> with a structural comparer to guarantee correctness.</remarks>
...
public sealed class Spec<S>(S initial)
{
    ...
    /// <summary>A deliberate defect scoped to steps applying the action named <paramref name="on"/>. On every step of that action the resulting state is replaced by <paramref name="perturb"/>. Analogous to the <c>on:</c> overload of <see cref="Spec{S}.Never(string, string, string, Func{S, S, bool})">Never</see>.</summary>
    ...
    /// <summary>A deliberate defect scoped to steps applying the action named <paramref name="on"/> where <paramref name="when"/> holds. Combines the action scope of the <c>on:</c> overload with the step predicate of the <c>when:</c> overload.</summary>
...
public static partial class Check
{
        ...
        int time = -1, int threads = -1)
    {
        spec.Validate();
        var counters = new SpecCounters(spec.Requirements.Count, spec.ArgPairs);
        // Generic types (ValueTuple`4, anonymous types) produce unreadable names; omit the type.
        var sutClause = typeof(TSut).Name.Contains('`') ? "" : $" of {typeof(TSut).Name}";
