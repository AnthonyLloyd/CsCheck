...
When coming up with a small number of simple examples consider if the test could be written for a range of examples.
...
A 'long time' could be 60 seconds since examples are run in parallel by default and often millions can be run in this time.
...
The generators created for a domain can be composed and reused across a number of tests e.g. serialization and domain logic.
...
- Serialization - the number of bugs seen in serialization code (looking at you json) is almost criminal given how easy it is to roundrip test serialization using random testing.
...
CsCheck has been used to help test serialization in Microsoft Orleans and Hager, and high performance immutable collections in ImTools.

It has given me the ability to write safe native code and interop, high performance collections, faster servers and much more.
It basically raised my game in terms of the quality of solutions I can attempt.